...
ShowPokedexData:
	...
	; External displays are always the normal DATA page and must not browse.
	xor a
	ld [wPokedexModeSelect], a
	ld [wMoveListCounter], a
...
ShowPokedexDataInternal:
	...
	; A advances text normally; B is the extra watched key. Left/Right are
	; added by DrawPokedexDataArrows only for the real Pokédex browser.
	ld a, B_BUTTON
	ld [wMenuWatchedKeys], a
...
.PrintDescription
	...
	; The flavor-text routine already handles every real description page
	; prompt. Do NOT add another prompt here: doing so creates a fake third
	; blank page after the final description page. hJoy5 contains the button
	; that ended the real text prompt, so hand it directly to navigation.
	ldh a, [hJoy5]
	ld b, a
.descriptionButtonTracking
	bit BIT_B_BUTTON, b
	jr nz, .exitDataPage
	; External displays (new catch/registration) must never turn a text-page
	; Left/Right press into Pokédex browsing. The text engine can leave the
	; button that ended the final description page in hJoy5, so gate it here.
...
.waitForButtonPress
	...
	jr .handlePageInput
.externalPageButtons
	; On a catch/registration entry, Left/Right is completely ignored.
